...
export const AdminAiInsightsPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setData(await analyticsService.getInsights(activeRestaurant.id, query));
    } catch (err: any) {
  ...
  useEffect(() => { fetchData(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [activeRestaurant?.id, period, date, startDate, endDate]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await analyticsService.exportInsights(activeRestaurant.id, query, format);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Sparkles className="w-6 h-6 text-amber-400" />AI Insights & Recommendations</h1>
          <p className="text-sm text-zinc-400 mt-1">Data-driven insights generated from {activeRestaurant?.name}'s real transaction data.</p>
        </div>
      ...
      {loading && !data ? (
        <div className="p-16 text-center text-zinc-500"><RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 text-amber-400" /><p className="text-sm">Generating insights...</p></div>
      ) : data ? (
        <div className="space-y-8">
          ...
          {(data.insufficientData || []).length > 0 && (
            <div className="p-4 rounded-xl bg-zinc-900/40 border border-zinc-800 text-xs text-zinc-500">
              {data.insufficientData.map((m: string, i: number) => <p key={i}>• {m}</p>)}
            </div>
          )}
        </div>
      ) : null}
